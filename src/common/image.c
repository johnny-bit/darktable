...
void dt_image_full_path(const int32_t imgid, char *pathname, size_t pathname_len, gboolean *from_cache)
{
  ...
  if(*from_cache)
  {
    ...
    dt_print(DT_DEBUG_IMAGEIO, "[dt_image_full_path] calling _image_local_copy_full_path\n");
...
static void _image_local_copy_full_path(const int32_t imgid, char *pathname, size_t pathname_len)
{
  ...
  *pathname = '\0';
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                              ...
                              -1, &stmt, NULL);
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
  dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] lesse if we've a row\n");
  if(sqlite3_step(stmt) == SQLITE_ROW)
  {
    char filename[PATH_MAX] = { 0 };
    char cachedir[PATH_MAX] = { 0 };
    dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] seems like we have a row!\n");
    g_strlcpy(filename, (char *)sqlite3_column_text(stmt, 0), pathname_len);
    dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] got filename: '%s'\n", filename);
    char *md5_filename = g_compute_checksum_for_string(G_CHECKSUM_MD5, filename, strlen(filename));
    dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] got filename md5: '%s'\n", md5_filename);
    dt_loc_get_user_cache_dir(cachedir, sizeof(cachedir));
    dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] cachedir: '%s'\n", cachedir);
    ...
    dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] pathname (oldtype): '%s'\n", pathname);
    ...
    if(!g_file_test(pathname, G_FILE_TEST_EXISTS))
    {
      ...
      dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] pathname (newtype): '%s'\n", pathname);
    }

    g_free(md5_filename);
  }
  sqlite3_finalize(stmt);
  dt_print(DT_DEBUG_IMAGEIO, "[_ilcfp] finished and cleaned up!\n");
}
